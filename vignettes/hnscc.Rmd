...
set.seed(1)
cols <- order(apply(Y,2,sd),decreasing = TRUE)
cols <- cols[1:1000]
res  <- Rtsne(as.matrix(Y[,cols]),normalize = TRUE)
colnames(res$Y) <- c("tsne1","tsne2")
pdat <- cbind(res$Y,info)
ggplot(pdat,aes(x = tsne1,y = tsne2,color = subject,shape = subtype)) +
  geom_point() +
